...
from modules.account.models.account_invoice_line import AccountInvoiceLine
...
class AccountInvoice:
    ...
    def _compute_amount(self):
        """Computes amount."""
        self.amount_total = round(sum(line.price_subtotal() for line in self.lines), DECIMALS_CALC)
    ...
    def add_lines(self, products_qty):
        ...
        if not isinstance(products_qty, (list, tuple)):
            logger.critical("Error! products_qty must be a sequence of sequences. Ex: ((product_soda, 2), (product_beer, 3))"
    ...
    def validate(self):
        ...
        if self.is_validated:
            raise AccountInvoiceException(f"Error! This invoice has already been validated!: {self.id}")
        elif not self.lines:
            raise AccountInvoiceException(f"Error! This invoice is empty; you cannot validate it! : {self.id}")
    ...
    def cancel(self):
        ...
        if self.is_validated:
            raise AccountInvoiceException(f"Error! Cannot cancel a validated invoice!: {self.id}")
    ...
    def pay(self):
        ...
        if self.is_paid:
            raise AccountInvoiceException(f"Error! This invoice has already been paid!: {self.id}")
        elif not self.is_validated:
            raise AccountInvoiceException(f"Error! Cannot pay a not validated invoice!: {self.id}")
        elif self.is_cancelled:
            raise AccountInvoiceException(f"Error! Cannot pay a cancelled invoice!: {self.id}")
    ...
    def __str__(self):
        res = [f"{'-' * 40}",
               ...
               f"           {self.client_email}\n"]
        res += [f"\t{'Product':15} {'Price':>6} {'Qty.':>7}",
                f"\t{'-' * 30}"]
        for line in self.lines:
            res += [f"\t{line}"]

        res += [f"\n  Total: {self.amount_total:8.2f}",
                f"{'-' * 40}"]
        return '\n'.join(res)
